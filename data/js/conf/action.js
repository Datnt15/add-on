...
//                action.updatepage(xhr.responseText);
        });
        return false;
    });
...
action.getVersion = function () {
    return '20140225'; // nam-thang-ngay
};
...
action.getHtml = function (boxComment) {
    ...
    s += '    <div style="padding-top: 2px;font-size: 10px;float: right;">        <div style="color: #999;">Tỉ giá ' + action.params.options.currency + ' hiện tại: <span id="rate_value">' + Number(action.params.rate).toMoney() + '</span></div><a href="' + config.service.website + '" data-spm-anchor-id="a312a.7728556.2015080704.1" style="color: #518cdc;text-decoration: initial;">Mua hàng tại Alodathang</a><br /><input id="addon_translate"  type="button" value="Dịch: Tiếng Việt" style="cursor: pointer;background:#0275d8;border:none; padding:5px 10px; color:#fff; text-transform:uppercase; margin-top:5px;-moz-border-radius:3px; -webkit-border-radius:3px; border-radius:3px;" /></div>';
    ...
    s += '    <a id="addon_view_cart" style="cursor: pointer;background: rgb(91, 192, 222);border: none;padding: 6px 10px;color: #fff;text-transform: uppercase;margin-top: 5px;-moz-border-radius: 3px;-webkit-border-radius: 3px;border-radius: 3px;">Xem giỏ hàng</a>';
    //s += '    <a id="demo01" href="#animatedModal">DEMO01</a><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#viewCartModel" style="cursor: pointer;background: rgb(91, 192, 222);border: none;padding: 6px 10px;color: #fff;text-transform: uppercase;margin-top: 5px;-moz-border-radius: 3px;-webkit-border-radius: 3px;border-radius: 3px;">Xem giỏ hàng</button>';
    s += '    <a href="' + config.service.cartLink + '" target="_blank" id="addon_view_link_cart" style="cursor: pointer;background: #0275d8;border: none;padding: 6px 10px;color: #fff;text-transform: uppercase;margin-top: 5px;display: block;width: 163px;text-align: center;-moz-border-radius: 3px;-webkit-border-radius: 3px;border-radius: 3px;">Thanh toán</a>';
    s += '  </div>';
...
action.showCart = function () {
    ...
    /*if (window.location.protocol == 'https:') {
        url = config.service.https.viewCart;
    }
    try {
        $.get(url, function (data) {
            action.updatepage(data);
        });
        ajax.submit(url, {}, function (xhr) {
            action.updatepage(xhr.responseText);
        });
    }
    catch (err) {
